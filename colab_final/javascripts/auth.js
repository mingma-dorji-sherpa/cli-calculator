...
document.addEventListener("DOMContentLoaded", () => {
    ...
    async function fetchUserProfile(provider, token) {
        try {
            ...
            const standardizedUser = {
                ...
                user_image: userData.picture, // Use Google image URL directly
                provider: "google"
            };
    ...
    async function saveUserToDatabase(user) {
        try {
            // Validate Google image URL
            let userImage = user.user_image;
            if (!userImage || !userImage.startsWith('https://')) {
                console.warn("Invalid Google image URL, using null in database");
                userImage = null; // Let backend handle default if needed
            }
            ...
            if (!checkResponse.ok || !checkResult) {
                throw new Error(`Failed to check user in database: ${checkResult.message || 'Unknown error'}`);
            }

            if (checkResult.success) {
                // User exists, update profile picture and provider if needed
                const updatedUser = {
                    username: checkResult.user.username,
                    email: checkResult.user.email,
                    user_image: userImage || checkResult.user.user_image, // Preserve existing image if new one is invalid
                    provider: "google"
                };
                ...
                if (!updateResponse.ok || !updateResult.success) {
                    throw new Error(updateResult.message || "Failed to update user in database");
                }

                // Use the image returned by the backend
                updatedUser.user_image = updateResult.user_image || userImage;
                return updatedUser;
            } else {
                ...
                const registerResponse = await fetch('http://localhost/Finalc/colab_final/connect.php', {
                    ...
                    body: JSON.stringify({
                        ...
                        user_image: userImage, // Send Google image URL
                        provider: "google"
                    })
                });
                ...
                return {
                    username: uniqueUsername,
                    email: user.email,
                    user_image: registerResult.user.user_image || userImage,
                    provider: "google"
                };
            }
        } catch (error) {
    ...
    async function fetchAndDisplayUserData(username) {
        try {
            ...
            const standardizedUser = {
                username: result.user.username,
                email: result.user.email,
                user_image: result.user.user_image ? (result.user.user_image.startsWith('http') ? result.user.user_image : `http://127.0.0.1:5500${result.user.user_image}`) : 'https://default-image-url.com/default.jpg',
