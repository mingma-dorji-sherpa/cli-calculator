document.addEventListener("DOMContentLoaded", function () {
    ...
    function fetchContactMessages() {
        fetch('http://localhost/Finalc/colab_final/contact.php', {
            ...
            .then(data => {
                ...
                let removedMessages = [];
                try {
                    removedMessages = JSON.parse(localStorage.getItem('removedMessages') || '[]');
                    if (!Array.isArray(removedMessages)) {
                        removedMessages = [];
                    }
                } catch (error) {
                    console.error('Error parsing removedMessages from localStorage:', error);
                    removedMessages = [];
                }

                const visibleMessages = data.messages.filter(message => !removedMessages.includes(String(message.id)));
                ...
                document.querySelectorAll('.remove-message').forEach(link => {
                    link.addEventListener('click', function (e) {
                        e.preventDefault();
                        const messageId = String(this.getAttribute('data-id'));

                        try {
                            removedMessages.push(messageId);
                            localStorage.setItem('removedMessages', JSON.stringify(removedMessages));
                            fetchContactMessages(); // Refresh the table
                            fetchTodaysMessages(); // Update today's messages count
                        } catch (error) {
                            console.error('Error updating removedMessages in localStorage:', error);
    ...
    // Function to fetch and display recent bookings with passenger details
                ...
                if (data.success) {
                    ...
                    if (data.bookings.length === 0) {
                        ...
                        data.bookings.forEach(booking => {
                            ...
                            const fullName = passengerDetails.length > 0 ? passengerDetails[0].fullName : 'N/A';
                            const passportNumber = passengerDetails.length > 0 ? passengerDetails[0].passport : 'N/A';
                            const contactNumber = passengerDetails.length > 0 ? passengerDetails[0].contact : 'N/A';
                            ...
                            row.innerHTML = `
                                ...
                                <td class="action">
                                    ...
                                    ${booking.status !== 'Cancelled' ? `<a href="#" class="cancel-booking" data-id="${booking.id}" data-flight-id="${booking.flight_id}" data-flight-type="${booking.flight_type}" data-seats-booked="${booking.seats_booked}">Cancel</a>` : ''}
                                </td>
                            `;
                                    ...
                                    .then(data => {
                                        if (data.success) {
                                            ...
                                            if (booking.passenger_details) {
                                                booking.passenger_details.forEach((passenger, index) => {
                                                    passengerDetails += `
                                                        Passenger ${index + 1}:
